# AME-V2-HV (High-Velocity) Optimizer
# ETF-only, no leverage/derivatives. Designed to push CAGR toward ~40% with pragmatic DD control.
# Python 3.10+
...
VIX_TICKER = "^VIX"  # only for regime; handled with ATR fallback
...
class Params:
    ...
    allow_three_when_spread: bool  # NEW: opportunistically hold #3
    spread_threshold1: float  # threshold #1 vs #2
    spread_threshold2: float  # threshold #2 vs #3
...
def run_strategy(data: Dict[str, pd.DataFrame], p: Params) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # HV mode: daily rebal by default, no slow-down
    ...
    for i, dt in enumerate(idx):
        ...
        # Defensive override (rare in HV but kept)
        if p.use_regime and p.defensive_override and regime.loc[dt, "risk_off"] == 1:
            asset = p.defensive_asset if p.defensive_asset in close.columns else (
                "SHY" if "SHY" in close.columns else BASE_SYMBOL)
        ...
        if p.allow_three_when_spread and len(elig) >= 3:
            # require both gaps to be decent
...
def fitness(res: Result) -> float:
    ...
    # looser penalty up to -40%, then harsher
    pen = 0.0
    if dd < -0.40:
        pen += (abs(dd) - 0.40) * 6.0
    # reward CAGR heavily, MAR modestly
...
def mutate(p: Params, rate: float = 0.18) -> Params:
    ...
    return Params(
        ...
        spread_threshold1=flip(p.spread_threshold1, [0.005, 0.0075, 0.01]),
        spread_threshold2=flip(p.spread_threshold2, [0.005, 0.0075, 0.01]),
        abs_mom_gate_base=flip(p.abs_mom_gate_base, [0.003, 0.004, 0.005]),
        abs_mom_gate_scale_vol=False,
        max_weight_min=0.80,
        max_weight_max=1.00,
        cash_buffer_min=0.02,
        ...
        slow_down_in_chop=False,
        defensive_override=True,
        defensive_asset=flip(p.defensive_asset, ["IAU", "GLD"]),
...
def save_results(all_results: List[Result], best: Result, tag: str) -> None:
    ...
    df.to_csv(f"{OUT_DIR}/optimizer_results_ame_v2_hv.csv", index=False)
    ...
    with open(f"{OUT_DIR}/optimizer_manifest_ame_v2_hv.json", "w") as f:
...
def main():
    ...
    save_results(all_results, best, tag="v2_hv")
    ...
    try:
        ...
        plt.title("AME-V2-HV Best Strategy vs VOO")
        plt.xlabel("Date");
        plt.ylabel("Equity")
        plt.tight_layout()
        fp = f"{OUT_DIR}/optimizer_ame_v2_hv_best_equity.png"
        plt.savefig(fp);
        plt.close()
        print(f"Saved: {fp}")
